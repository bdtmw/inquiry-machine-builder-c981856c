    ...
    <div className="container mx-auto max-w-3xl text-center relative z-10">
      <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="space-y-8">
        ...
        <div className="flex flex-wrap justify-center gap-4 pt-2">
          ...
          <Button variant="hero-secondary" size="lg" className="text-base px-10 py-6" onClick={scrollToForm}>
            Get Started
          </Button>
        </div>
        <div className="flex flex-wrap justify-center gap-8 pt-6 text-sm text-hero-foreground/40">
          <span className="flex items-center gap-2"><Shield className="h-4 w-4 text-accent" /> 30-day money-back guarantee</span>
          <span className="flex items-center gap-2"><Zap className="h-4 w-4 text-accent" /> Setup in 48 hours</span>
        </div>
      </motion.div>
    </div>
  </section>
